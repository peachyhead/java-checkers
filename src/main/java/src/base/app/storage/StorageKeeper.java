...
public class StorageKeeper {
    ...
    public <T extends IIdentifiable & IInitializable> void bindStorage(Class<T> type, Storage<T> storage) {
        var item = new StorageMapItem(type, storage);
        storageMap.add(item);
    }

    public static <T extends IIdentifiable & IInitializable> Storage<T> getStorage(Class<T> tClass) {
        try {
